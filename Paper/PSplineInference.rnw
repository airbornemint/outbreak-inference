...
A \textit{generalized additive model} (GAM --- sometimes known as a generalized linear model, GLM) provides a framework for modeling a response variable $y$, using predictor variables $x_i, i=1, \hdots, p$ with associated non-linear predictor functions $f_i, i=1, \hdots, p$ and a function $g$ (\textit{link function}) such that the expected value $E(y)$ is given by:
...
Splines can be used as predictor functions $f_i$ in a GAM. Expressed in terms of B-splines, $f_i = \sum_j \beta_{i,j} B_{i,j}$, and the GAM takes the form:
...
In this model $\beta_i \sim N(\mu_i, \sigma_i)$, with the best-fit corresponding to $\beta_i = \mu_i$. Since $E(I)$ is a function of $\beta_i$ and the distribution of each $\beta_i$ is known from the model, we can sample $I(t)$ indirectly by sampling $\beta_i$ and calculating $I(t) = \exp(\beta_0 + \sum_i \beta_i B_i(t)$. Figure \ref{fig:samplePredMini} shows $\Sexpr{sampleNsimMini}$ different $I(t)$ curves produced by independently sampling each $\beta_i$  $\Sexpr{sampleNsimMini}$ times.
