...
If we assume that $\beta_k \sim \Norm(\mu_k, \sigma_k)$, we can use the P-spline GAM to obtain a Bayesian confidence interval across the entire function $y(t)$ by sampling $\beta_i$ and calculating $y(t) = \exp(\beta_0 + \sum_k \beta_i B_k(t)$. This confidence interval has been shown to closely approximate the nominal frequentist confidence interval across the entire function for non-linear data following Poisson, binomial, and gamma distributions\cite{MARRA:2012fi}. Fig~\ref{fig:pspline-samplePredMini} shows \Sexpr{sampleNsimMiniText} $y(t)$ curves produced by independently sampling each $\beta_i$ \Sexpr{sampleNsimMiniText} times.
