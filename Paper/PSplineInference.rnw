...
To demonstrate the validity of our method, and specifically that the 95\% CI estimates of the onset/offset time and the preventable fraction were correct, we performed a simulation study. We began by generating $N_\mathit{season} = \Sexpr{simNTruth}$ RSV seasons. Each season was generated as follows:
...
\item Fit a $\log$-linked P-spline to $I^*(t)$ to obtain true instantaneous incidence $I_0(t)$. 
\end{enumerate}
...
\item Absolute bias: the mean difference between point estimates and the true value
\item Relative bias: for onset and offset only, the mean difference between point estimates and the true value divided by $t_\mathit{off} - t_\mathit{on}$
\end{itemize}
...
Power (?)   		            &  \Sexpr{round(100*validationResults$summary$onset.good, 2)}                   & \Sexpr{round(100*validationResults$summary$offnset.good, 2)}                  & TBD                   \\ \hline
Mean bias	                    &  \Sexpr{round(validationResults$summary$onset.bias, 2)}                       & \Sexpr{round(validationResults$summary$offset.bias, 2)}                       & TBD                   \\ \hline
Bias standard error	            &  \Sexpr{round(validationResults$summary$onset.bias.se, 2)}                    & \Sexpr{round(validationResults$summary$offset.bias.se, 2)}                    & TBD                   \\ \hline
...
Mean fractional bias	        &  \Sexpr{round(validationResults$summary$onset.bias.frac, 2)}                  & \Sexpr{round(validationResults$summary$offset.bias.frac, 2)}                  & ---                   \\ \hline
Fractional bias standard error	&  \Sexpr{round(validationResults$summary$onset.bias.frac.se, 2)}               & \Sexpr{round(validationResults$summary$offset.bias.frac.se, 2)}               & ---                   \\ \hline
