...
A \textit{cyclic spline} of order $n$ is a piecewise polynomial function $f = f(x) = f(x - T) : \mathds{R} \Rightarrow \mathds{R}$, with knots at $t_a = t_1 \leq t_2 \leq \dots \leq t_k = t_a + T = t_b$. A maximally smooth cyclic spline is continuous in $n - 1$ derivatives throughout $\mathds{R}$. Use of cyclic splines is often necessary in infectious disease modeling because non-cycling splines do not guarantee smoothness at $T_a \pm k T$ when extended to $\mathds{R}$.
...
We begin by fitting a P-spline GAM to these observations, using incidence $I(t)$ as the model response variable, $\log$ link function, and a 20-dimensional cubic basis spline with 2nd degree smoothing penalty as the only model term:

\begin{align*}
    \log(\hat{I}(t)) &= \beta_0 + \beta_1 B_1(t) + \beta_2 B_2(t) + \dots + \beta_20 B_20(t)
\end{align*}
...
\item Randomly choose season start ($t_\mathit{start} \sim ?$), season end ($t_\mathit{start} \sim ?$), and peak incidence ($I_\mathit{max} \sim ?$), and calculate \begin{equation*}
 ...
 0 &\text{otherwise}
 \end{cases}
\end{equation*} This gives us an easy way to roughly control the overall shape of the simulated season, after which we
...
The full documentation for our \texttt{pspline.inference} R package, including detailed examples, is available as part of the package, which is available at (TODO: release on CRAN, currently at gitlab.com/airbornemint/pspline.inference); here we only present a brief overview of the two main functions in the package to highlight its ease of use.
...
\begin{enumerate}
\item \texttt{model}: a GAM model, 
\item \texttt{predictors}, a data frame of predictor values
\item \texttt{fun.outcome(data)}: a function which receives a data frame \texttt{data} containing the model-predicted time series of disease incidence and computes a scalar outcome measure $O$ for the given time series
\end{enumerate}
...
\begin{enumerate}
\item \texttt{fun.truth()}: a function which generates a true time series of disease incidence,
\item \texttt{fun.obs(truth)}: a function which receives a data frame \texttt{truth} containing a true time series of disease incidence and returns a data frame of observed incidences
\item \texttt{fun.model(observations)}: a function which receives a data frame \texttt{observations} of observed values of disease incidence and returns a GAM model for those observations, and
\item \texttt{fun.outcome(data)}: a function which receives a data frame \texttt{data} containing a time series of disease incidence (true or observed) and returns the scalar outcome measure $O$ for the given time series
\end{enumerate}
...
where \texttt{n.truth} is the number of true time series generated for the simulation study and \texttt{n.obs} is the number of observed time series generated for each true time series. 
