...
\title{Use of penalized basis splines in estimation of characteristics of seasonal and sporadic infectious disease outbreaks}
\def\headertitle{Use of P-splines in estimation of characteristics of infectious disease outbreaks}
...
Most of the widely used methods of modeling outcomes of infectious disease outbreaks begin with a generalized linear model (GLM) of disease incidence, followed by estimation of the outcome measure of interest from incidence estimates. Calculation of outcome measures of clinical and public health significance is typically non-trivial, and consequently only point estimates (rather than interval estimates) of those measures can be made from the model. Motivated by respiratory syncytial virus (RSV) seasonality, we developed a general method for obtaining interval estimates of outcome measures of seasonal and sporadic infectious disease outbreaks using multivariate sampling of a generalized additive model with penalized basis splines. Our approach provides robust interval estimates regardless of the complexity of the calculations of the outcome measures, and it generalizes to other systems (including outbreaks of other infectious diseases). Here we present our general approach, its application to RSV, and an R package that provides a convenient interface for conducting and validating this type of analysis in other areas.
...
Analysis of infectious disease dynamics often concerns itself with inferences about time --- for example, the start and the end of seasonal outbreaks, duration of outbreaks, or optimal timing of prophylaxis or treatment. The typical mathematical model used for these analyses treats observed disease incidence as the outcome variable in a generalized linear model (GLM) or a generalized additive model (GAM).

One of the principal limitations of GLM/GAM-based estimation methods is that they are only able to provide interval estimates for model parameters, the model response variable, and quantities trivially computed from those. Any quantity that is non-trivially computed from model parameters and response variables is only amenable to point-estimation, not interval estimation. Unfortunately, this limitation applies to many quantities of clinical and public health interest in infectious disease dynamics, because time-domain quantities (such as season start and end) have a complex computational relationship to the model parameters and response.

These challenges are exemplified by respiratory syncytial virus (RSV) --- a respiratory infection with annual or biennial seasonal pattern that is a major infectious cause of infant morbidity and mortality globally. In some countries, high-risk infants are administered antibody prophylaxis throughout the period of high RSV incidence. The therapy is expensive and the duration of antibody protection is short-lived, so prophylaxis administration needs to be timed with the RSV season to provide optimal protection. Therefore, estimating the timing of initiation of the RSV season in different locations is important. A variety of methods have been employed for this purpose,\cite{Midgley:2017bo} --- some more ad-hoc than others, and showing different degrees of sensitivity to the characteristics of the data outside the RSV season. 

Our approach to this challenge is to fit a smooth curve through the observed disease incidence time series, and infer time-domain features of the outbreak from this curve. The fitting curves we use are penalized basis splines (P-splines), which have been shown to have several qualities desirable in the analysis of experimental data, including computational compactness and efficiency, and well-understood statistical behavior.\cite{Eilers:1996kz} In this study, we show how GAM with P-splines can be used to fit smooth curves to infectious disease incidence data, and then used to make interval estimates of characteristics of RSV seasonal outbreaks, such as the time of their start and end.
...
A \textit{generalized additive model} (GAM --- sometimes known as a generalized linear model, GLM) provides a framework for modeling a response variable $y$, using potentially multiple predictor variables $x_i, i=1, \hdots, p$ with associated non-linear predictor functions $f_i, i=1, \hdots, p$ and a function $g$ (\textit{link function}) such that the expected value $\hat{y}$ of $y$ is given as:

\begin{equation*}
    g(\hat{y}) = \beta_0 + f_1(x_1) + f_2(x_2) + \dots + f_p(x_p)
\end{equation*}

\subsubsection{Basis splines}

A \textit{spline} of order $n$ is a piecewise polynomial real function of degree $n - 1$ on interval $[t_a, t_b]$; the points at which its polynomial segments join are known as \textit{knots} $k_i$. Owing to its construction, a spline of order $n$ has up to  $n - 1$ continuous derivatives throughout $[t_a, t_b]$; for our analysis, we only consider splines with exactly $n - 1$ continuous derivatives.

Given a set of knots $k_i$, we can construct a unique set of splines of order $n$ (with those knots) known as the \textit{basis splines} (B-splines) and denoted with $B_i$. All splines with knots $k_i$ can be represented as a linear combination of the basis splines with the same knots.
...
\begin{aligned}
    ...
    &+ \dots \\
    &+ \beta_{p,1} B_{p,1}(x_p) + \beta_{p,2} B_{p,2}(x_p) + \dots
\end{aligned}\label{eqn:model}
\end{equation}

This model can be fitted to data simply by minimizing $\Vert y - \hat{y} \Vert^2$; however, this approach often leads to overfitting, especially with a larger number of knots. To mitigate this, the model can be modified to instead minimize $\Vert y - \hat{y} \Vert^2 + P$, where $P$ is a \textit{penalty function} that measures non-smoothness of the model response variable. This approach balances closeness of fit with smoothness. B-splines used in a model with a penalty function are known as \textit{penalized B-splines} (P-splines). A variety of penalty functions have been defined; the most common ones use $k$-th order differences between the model and the data, for some small integer $k$ (often $k = 2$). As with general cyclic splines, \textit{cyclic P-splines} are an extension of P-splines to periodic functions.
...
The principal questions of our RSV work were:
...
Once we constructed a P-spline GAM (Equation \ref{eqn:model}) and fit it to our incidence data, we could sample the model parameters $\beta_{i,j}$, use them to estimate the  model response variable (in our case, RSV incidence), and therefrom estimate the outcome measures of interest. By way of example, consider the (hypothetical) RSV season shown in Figure \ref{fig:sampleCases}. 
...
To estimate the season onset and offset for this RSV season, we begin by fitting a P-spline GAM to the observations, using incidence $I(t)$ as the model response variable, $\log$ link function, and a 20-term cubic basis spline (with 2nd degree smoothing penalty) as the only model term:

\begin{align*}
    \log(\hat{I}(t)) &= \beta_0 + \beta_1 B_1(t) + \beta_2 B_2(t) + \dots + \beta_{20} B_{20}(t)
\end{align*}

The best fit produced by this model is shown in Figure \ref{fig:sampleBestFit}.
...
In this model $\beta_i \sim N(\mu_i, \sigma_i)$, with the best-fit corresponding to $\beta_i = \mu_i$. Since $\hat{I}$ is a function of $\beta_i$ and the distribution of each $\beta_i$ is known from the model, we can sample $I(t)$ indirectly by sampling $\beta_i$ and calculating $I(t) = \exp(\beta_0 + \sum_i \beta_i B_i(t)$. Figure \ref{fig:samplePredMini} shows $\Sexpr{sampleNsimMini}$ different $I(t)$ curves produced by independently sampling each $\beta_i$  $\Sexpr{sampleNsimMini}$ times.
...
Next we turn to our outcome measure of interest: time of outbreak onset ($t_\mathit{on}$) and offset ($t_\mathit{off}$), defined by:
...
Sampling a large number of $t_\mathit{on}$ and $t_\mathit{off}$ yields an approximation of the unknown distributions of $t_\mathit{on}$ and $t_\mathit{off}$. In our example, sampling $N = \Sexpr{sampleNsimFull}$ times yields the distributions of $t_\mathit{on}$ and $t_\mathit{off}$ shown in Figure \ref{fig:samplePredOnsetFull}. From these distributions, we can calculate our desired point estimates (median) and interval estimates (quantiles): $\hat{t}_\mathit{on} = \Sexpr{round(estThresholdsFull['onset.median'], 2)}$ weeks ($\Sexpr{level*100}$\% CI: $\Sexpr{round(estThresholdsFull['onset.lower'], 2)}$ - $\Sexpr{round(estThresholdsFull['onset.upper'], 2)}$ weeks), $\hat{t}_\mathit{off} = \Sexpr{round(estThresholdsFull['offset.median'], 2)}$ weeks ($\Sexpr{level*100}$\% CI: $\Sexpr{round(estThresholdsFull['offset.lower'], 2)}$ - $\Sexpr{round(estThresholdsFull['offset.upper'], 2)}$ weeks).
...
Our simulation study found that the true value of season onset was included in the estimated $95$\% confidence interval in $\Sexpr{round(100*validationResults$summary$onset.good, 2)}$\% of the simulations. The distribution of the true season onset's percentile in each simulation's distribution of season onset estimates is shown in Figure \ref{fig:onsetQuantilesFull}.

\begin{figure}
\begin{center}
\input{\figuresDir/onsetQuantilesFull.tex}
...
We similarly found that the true value of season offset was included in the estimated $95$\% confidence interval in $\Sexpr{round(100*validationResults$summary$offset.good, 2)}$\% of the simulations. The distribution of the true season offset's percentile in each simulation's distribution of season offset estimates is shown in Figure \ref{fig:offsetQuantilesFull}.

\begin{figure}
\begin{center}
\input{\figuresDir/offsetQuantilesFull.tex}
...
Of note for onset and offset estimates is that our method tends to skew high (overestimate) for onset and low (underestimate) for offset; this skew is also seen in Figure \ref{fig:samplePredOnsetFull}. This is due to the asymmetric variability of the P-splines; there is much less variability in RSV incidence outside of the RSV season than during the RSV season, and therefore there is more variability on the high end of onset estimates and the low end of offset estimates, which is reflected in a skewed estimate distribution.

Finally, we found that the true value of the protected fraction was included TODO %in the estimated $95$\% confidence interval in $\Sexpr{round(100*validationResults$summary$protectedFraction.good, 2)}$\% of the simulations. The distribution of the true season offset's percentile in each simulation's distribution of season offset estimates is shown in Figure \ref{fig:protectedQuantilesFull}.

%\begin{figure}
%\begin{center}
%\input{\figuresDir/protectedQuantilesFull.tex}
%\caption[P-spline samples approximating of RSV incidence]{Incidence of RSV-associated hospitalizations, with sampled approximating $P$-splines ($N=\Sexpr{sampleNsimMini}$)}
...
That said, the greater significance of our results rests neither specifically with RSV nor specifically with those outcome measures. Our estimation method generalizes to other systems (such as outbreaks of other infectious disease), as well as other outcome measures.
