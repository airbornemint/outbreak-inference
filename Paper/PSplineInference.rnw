...
Analyses of infectious disease dynamics often make inferences based on timing for various quantities of clinical and public health interest; for example, the start and the end of seasonal outbreaks, duration of outbreaks, and/or optimal timing of prophylaxis or treatment. Some aspects of timing can be estimated ad-hoc directly from disease incidence data; more typically, a regression model (e.g., a generalized linear model (GLM) or a generalized additive model (GAM)) is fitted to a time series of observed disease counts, and the calculations of the quantities of interest are made based on combinations of parameters estimated from the fitted model. 
...
To estimate the onset and offset for this RSV season, we begin by fitting a cyclic P-spline GAM to the observations, using incidence $y(t)$ as the model response variable, $\log$ link function, and a 20-term cubic spline basis $B_k$ as the only model term:

\begin{equation}
\begin{aligned}
    y(t) &\stackrel{\text{ind}}{\sim} \text{Poisson}(\lambda(t)) \\
...
In this model $\beta_k \sim \Norm(\mu_k, \sigma_k)$, with the best-fit corresponding to $\beta_k = \mu_k$. Since $y(t)$ is a function of $\beta_k$ and the distribution of each $\beta_k$ is known, we can sample $y(t)$ indirectly by sampling $\beta_i$ and calculating $y(t) = \exp(\beta_0 + \sum_k \beta_i B_k(t)$. Fig~\ref{fig:pspline-samplePredMini} shows \Sexpr{sampleNsimMiniText} $y(t)$ curves produced by independently sampling each $\beta_i$ \Sexpr{sampleNsimMiniText} times.
...
The number of seasons was chosen to balance power of our simulation and its computational demand, as the simulation study is the most time-consuming component of our approach. The ranges of the parameters for the simulated seasons were chosen to include the observed characteristics in our dataset of RSV-related hospitalizations in Conneticut. 
...
To evaluate the results of our method, we considered three measures of robustness: 
...
The process of performing a simulation study is straightforward, but the measures of robustness it yields must be interpreted in the context of the research at hand. In our particular case, we found that the onset estimates had a slight negative bias (underestimate), whereas the offset estimates had a positive bias (overestimate) of equal magnitude (Table~\ref{table:results}), due to the inherently asymmetric shape of the outbreak curve at onset and at offset. Furthermore, the MAE for onset/offset estimates ($\sim0.15$ weeks) did not affect the conclusions of our RSV research as it was small compared to effect sizes (1-2 weeks). 
