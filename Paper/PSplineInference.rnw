...
\begin{subfigure}[t]{0.45\textwidth}
    ...
    \caption[P-spline samples approximating of RSV incidence]{\Sexpr{toTitleCase(sampleNsimMiniText)} different sampled P-spline models of RSV incidence}
    \label{fig:samplePredMini}
\end{subfigure}
\begin{subfigure}[t]{0.45\textwidth}
    \paperincludegraphic{\figuresDir/samplePredOnsetMini}
    \caption[Estimates of RSV season onset and offset]{\Sexpr{toTitleCase(sampleNsimMiniText)} different computed estimates of RSV season onset and offset.}
...
In this model $\beta_k \sim \Norm(\mu_k, \sigma_k)$, with the best-fit corresponding to $\beta_k = \mu_k$. Since $y(t)$ is a function of $\beta_k$ and the distribution of each $\beta_k$ is known, we can sample $y(t)$ indirectly by sampling $\beta_i$ and calculating $y(t) = \exp(\beta_0 + \sum_k \beta_i B_k(t)$. Fig~\ref{fig:samplePredMini} shows \Sexpr{sampleNsimMiniText} $y(t)$ curves produced by independently sampling each $\beta_i$ \Sexpr{sampleNsimMiniText} times.
