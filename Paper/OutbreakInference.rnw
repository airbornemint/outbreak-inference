...
In the analysis that motivated the development of this approach, we were analyzing the seasonal patterns of RSV infections. Consider the RSV season, based on real data from Connecticut, shown in Figure \ref{fig:sampleCases}.
...
As before, we sample $\beta_{i,j}$ to obtain a sampling distribution of $\widehat\PF$ and thereby calculate interval estimates of $\PF$.
