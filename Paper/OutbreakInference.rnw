...
Seasonal and sporadic outbreaks of infectious diseases are usually amenable to generalized linear modeling (GLM) and generalized additive modeling (GAM). The existing R packages for GAM and GLM provide a convenient means of making interval estimates of disease incidence at specific points in time. However, outbreak outcome measures that are of interest in clinical and public health practice often require estimation of incidence accumulated over periods of time. No comparably convenient R package exists for interval estimation of outcome measures that are derived from cumulative disease incidence. We describe a method of obtaining such estimates using GAM with penalized basis splines (P-splines), and an R package providing a convenient interface for performing such analysis.
...
B-spline smoothness properties are one-sided at the endpoints of their domain. As a result, if B-spline approximation is applied to one period of the periodic function $y(t) = y(t - T)$ --- which often arises in analysis of seasonal infectious disease patterns --- $\hat y$ is not guaranteed to be smooth throughout its domain. We can regain smoothness in these situations by using \textit{cyclic} B-splines, which are B-splines additionally constrained to guarantee smoothness when the time domain is mapped to a cycle.
...
\begin{align*}
    g(\hat{y}) = \beta_0 &+ \beta_{1,1} B_{1,1}(x_1) + \beta_{1,2} B_{1,2}(x_1) + \dots \\
    &+ \beta_{2,1} B_{2,1}(x_2) + \beta_{2,2} B_{2,2}(x_2) + \dots \\
    &+ \dots \\
    &+ \beta_{n,1} B_{n,1}(x_n) + \beta_{n,2} B_{n,2}(x_n) + \dots
\end{align*}

where $B_{i,j}$ are basis spline functions and $\beta_{i,j}$ are model parameters.

However, it can be difficult to avoid overfitting in this model. This can be addressed by adding a penalty term to the model, so that instead of simply maximizing fit by minimizing $\Vert y - \hat{y} \Vert^2$, the model balances fit and smoothness by minimizing $\Vert y - \hat{y} \Vert^2 + P$, where $P$ is a function that measures non-smoothness of the model. B-splines used in this way are known as penalized B-splines (``P-splines''). A variety of penalty functions have been defined; the most commonly used penalty functions use $k$-th order differences between the model and the data, for some small integer $k$. As with B-splines, P-splines constrained to guarantee smoothness throughout the domain of a periodic function are known as \textit{cyclic} P-splines.
...
In the analysis that motivated the development of this approach, we were analyzing the seasonal patterns of RSV infections. For example, consider a hypothetical RSV season shown in Figure \ref{fig:sampleCases} (for privacy reasons, no actual patient data is used in our examples). 
