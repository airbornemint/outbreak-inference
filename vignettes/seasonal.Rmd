...
import::from(ggplot2, ggplot, aes, geom_ribbon, geom_line, geom_point, geom_density, theme_light, scale_y_continuous, scale_x_continuous, scale_colour_manual, scale_fill_manual, scale_size_manual, scale_linetype_manual, labs, coord_cartesian, theme, element_blank, element_text, element_rect, guide_legend)
...
basePlot <- function(data, legends) {
  ...
  legendLabels = c(
    ...
    supplyEnd="End of supply (estimated)"
  )
...
obs$cumcases <- cumsum(obs$cases)
obs$cumcases.relative <- obs$cumcases / max(obs$cumcases)
```
...
basePlot(cumCasesRelEst, c("casesObs", "casesEst")) +
  geom_ribbon(aes(x=time, ymin=cumcases.relative.lower, ymax=cumcases.relative.upper, fill="casesEst")) +
  geom_point(data=obs, aes(x=time, y=cumcases.relative, color="casesObs", size="casesObs")) +
...
basePlot(seriousEst, c("casesObs", "casesEstMedian", "seriousEstMedian")) +
  geom_line(aes(x=time, y=cumseriouscases.median, color="seriousEstMedian")) +
  geom_line(data=cumCasesEst, aes(x=time, y=cumcases.median, color="casesEstMedian")) +
  geom_point(data=obs, aes(x=time, y=cumcases, color="casesObs", size="casesObs")) +
  labs(y="Cases")
```
...
```{r estimate supply duration, message=FALSE, warning=FALSE}
...
kable_styling(
  kable(
    supplyDurationEst, 
    caption="Estimated time before medication supply is exhausted", 
...
basePlot(seriousEst, c("casesObs", "casesEst", "supplyEnd")) +
  geom_ribbon(data=cumCasesEst, aes(x=time, ymin=cumcases.lower, ymax=cumcases.upper, fill="casesEst", color="casesEst")) +
  geom_point(data=obs, aes(x=time, y=cumcases, color="casesObs", size="casesObs")) +
  geom_density(
